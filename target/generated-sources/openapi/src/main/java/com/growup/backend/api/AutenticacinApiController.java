...
public class AutenticacinApiController implements AutenticacinApi {
    ...
    private final AutenticacinApiDelegate delegate;

    public AutenticacinApiController(@Autowired(required = false) AutenticacinApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new AutenticacinApiDelegate() {});
    }

    @Override
    public AutenticacinApiDelegate getDelegate() {
        return delegate;
    }
